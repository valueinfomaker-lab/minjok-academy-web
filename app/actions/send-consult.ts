...
export async function sendConsult(formData: FormData): Promise<SendResult> {
  ...
  try {
    // Apps Script doPost responds 302 → echo URL on success.
    // Following that redirect is unreliable (token-bound, often 4xx for
    // server-side fetch), but the 302 itself proves the script executed
    // and appendRow ran. Treat 2xx and 3xx as success.
    ...
    console.error("sheet webhook unexpected status", res.status);
